...
Adapted for Llama-3.1-Nemotron-Nano-VL-8B-V1 model
...
class NemotronSTREvaluator:
    def __init__(self, model_name: str = "nvidia/Llama-3.1-Nemotron-Nano-VL-8B-V1", 
                 device: str = "cuda", prompt: str = None, case_sensitive: bool = False,
        ...
        try:
            ...
            # Try to load model with different configurations to handle the flash attention issue
            try:
                ...
                print(f"Failed to load with eager attention, trying with default settings: {e}")
                self.model = AutoModel.from_pretrained(
                    model_name, 
                    trust_remote_code=True, 
                    device_map=device
            ...
            self.image_processor = AutoImageProcessor.from_pretrained(model_name, trust_remote_code=True, device=device)
    ...
    def predict_text(self, image: Image.Image) -> str:
        """Generate text prediction for given image using Nemotron model"""
        try:
            # Process image
            ...
            # Generate prediction using the chat interface
    ...
    def predict_batch(self, images: list) -> list:
        """Generate text predictions for multiple images (batch processing for Nemotron)"""
        if not images:
            return []
        
        results = []
        # For Nemotron model, we'll process images one by one in a loop
        # since the model interface doesn't support true batch processing
        for image in images:
    ...
    def save_detailed_results(self, results: dict, output_file: str):
        ...
        with open(output_file, 'w', encoding='utf-8') as f:
            # Header
            f.write("="*100 + "\n")
            f.write("STR Evaluation Results - Llama-3.1-Nemotron-Nano-VL-8B-V1\n")
            f.write("="*100 + "\n")
...
def main():
    parser = argparse.ArgumentParser(description='STR Evaluation for LMDB datasets with Nemotron')
    ...
    parser.add_argument('--model_name', type=str, default="nvidia/Llama-3.1-Nemotron-Nano-VL-8B-V1",
    ...
    if lmdb_paths:
        ...
        print("Loading model...")
        evaluator = NemotronSTREvaluator(
        ...
        with open(summary_file, 'w', encoding='utf-8') as f:
            f.write("STR OCR Evaluation Summary - Llama-3.1-Nemotron-Nano-VL-8B-V1\n")
            f.write("=" * 60 + "\n\n")
            ...
            if total_samples > 0:
                ...
                f.write(f"OVERALL         : {total_correct:>4}/{total_samples:>4} = 0.00%\n\n")
